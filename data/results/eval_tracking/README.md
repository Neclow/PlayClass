...
1. **Prepare** (`pixi run -e tracker prepare-tracker`)
   - `build-manifest` — ranks 30 candidate videos by difficulty, selects 5.
   - `select-frames` — picks annotation keyframes (chunk-guided + occlusion + uniform).
2. **Annotate** — manual bounding-box annotation in CVAT on the selected keyframes.
3. **Score** (`pixi run -e tracker score-tracker`)
